...
아래는 전부 **실제 실행 결과**다 (2026-09-09, 키워드 7개 / 최근 24시간 / 111건 수집).

### ① 오늘의 이슈 — 메인이자 승인 지점

![오늘의 이슈](docs/screenshots/1_오늘의_이슈.png)

111건이 64묶음으로 접혔다. 라벨·중요도·근거가 이미 채워진 상태로 나오고, 사람은 틀린 것만 드롭다운으로 고친 뒤 [확정]을 누른다. 상단에 수집 건수·묶음 수·토큰·비용·상태가 함께 뜬다.
...
- `search_news` 111건 8.8초 → `lookup_history` → `judge` 배치 반복
- 배치마다 입출력 토큰과 소요시간 (`in=784 out=172 · 17.9초`)
...
세트마다 키워드·주제 설명·중요도 기준을 따로 갖는다. 수정률이 20%를 넘으면 기준 점검을 권고한다.
...
```
[고정]  수집 → 규칙 클러스터링 → 이력 대조 → 라벨 판정(유사도)
[자율]  중요도 판정 → 정보 부족 판단 → 본문 조회 → 재판정 → 다음 행동 결정
[고정]  결과 표시 → 사람 승인 → 확정 저장
```

틀릴 여지가 없는 구간은 코드가 결정론적으로 처리하고, 판단이 필요한 곳에서만 에이전트가 돈다.

**종료 조건** — 묶음당 반복 3회 / 본문 조회 3건 / 5분 / 50,000 토큰.
상한 도달은 실패가 아니라 기록되는 상태이며, 미판정 묶음도 `보류` 표시로 목록에 남는다. `--resume`으로 이어서 진행한다.
...
43개. 핵심은 이 프로젝트의 출발점이 되는 주장을 고정한 것이다.
...
`qwen3.5:2b` 기준 111건 → 64묶음에서:

```
최우선 0  ·  필수 36 (56%)  ·  참고 28
```

- **등급 인플레이션** — 절반 이상이 `필수`라 정렬해도 36줄을 봐야 한다
- **환각** — "2030 청년 제주살이하며 농촌 체험"에 "B·법·제도 변경" 근거를 붙이는 식
- **지시 불이행** — reason 25자 제한을 반복 무시

프롬프트를 두 차례 고쳤으나 오류가 A등급에서 C등급으로 옮겨갔을 뿐이다. **2B 모델의 한계로 판단한다.** 상세는 PRD §8.5.

## 알려진 한계

- **CPU 추론이 느리다.** GPU 없는 환경에서 64묶음 판정에 약 11분(배치당 29초). 규칙 클러스터링이 LLM 입력을 크게 줄여 이 정도가 나온다
...
**1. 모델 비교 — 가장 시급 (PRD §8.3 나)**

현재 2B 결과가 baseline으로 확보되어 있다. 비교군만 붙이면 표가 완성된다.

```bash
ollama pull qwen3:4b                                          # 약 2.5GB
python eval/run_eval.py model --models qwen3.5:2b,qwen3:4b    # 실행당 10~20분
```

Anthropic API를 비교군에 넣으려면 `core/llm.py`의 `judge()`에 provider 분기를 추가한다(약 15줄). 월 1,500원 수준이며 판정이 10초 안쪽으로 끝난다.

**2. 누락률 측정 (수용 기준 3번)**

`streamlit run app.py` → 며칠 검토·확정 → ③ 화면의 수정률 지표로 확인한다. 사람이 고친 기록이 자동으로 쌓인다.

**3. 프롬프트 개선 (PRD §8.3 다)**

```bash
python eval/run_eval.py prompt      # 기준 유무 비교
```

few-shot 예시 주입은 ③ 화면에 쌓인 수정 이력에서 뽑아 쓴다.

**4. 48/72시간 창에서 상한 도달 확인**

24시간에서는 1,000건 상한에 닿지 않았다. 창을 넓히면 `사회적기업` 등이 닿는지 확인한다.

### 아직 안 한 것

- **네이버 API 키 재발급** — 현재 키는 노출 이력이 있다
- 게시판 감시, n8n 연동, 자동 실행 (설계만 되어 있고 구현 전)
