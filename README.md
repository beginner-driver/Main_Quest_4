...
**종료 조건** — 묶음당 반복 3회 / 본문 조회 3건 / 30분 / 50,000 토큰.
...
51개. 핵심은 이 프로젝트의 출발점이 되는 주장을 고정한 것이다.
...
| 3 | **누락률 10% 이하** | **미측정** — 확정 0건 |
| 4 | 수집 재현율 개선 확인 | **통과** (+70%) |
| 5 | 도구 호출 이유·결과·시간 기록 | **통과** (23단계 전부) |
| 6 | 중단 후 재개 | **통과** (`--resume`으로 20묶음 마저 판정) |
| 7 | 검토 시간 단축 실측 | **미측정** |

통과 3 / 부분 2 / 미측정 2. 미측정 2건은 사람이 며칠 운영해야 나오는 값이며, 측정 장치는 구현되어 있다. 상세는 PRD §10.1.
...
~~**1. 모델 비교**~~ ✅ **완료 (2026-09-10)** — `qwen3:4b`로 교체. 위 [모델 비교](#판정-품질--모델-비교로-해소) 참조.

**1. 누락률 측정 — 이제 가장 시급 (수용 기준 3번)**

모델 교체로 등급 분포는 좋아졌지만 **그 판정이 옳은지는 아직 아무도 확인하지 않았다.** 이게 남은 가장 큰 공백이다.
...
며칠 검토·확정하면 ③ 화면에 수정률이 쌓인다. "에이전트가 참고로 내렸는데 내가 최우선으로 올린" 건수가 곧 누락률이다.
...
few-shot 예시 주입은 ③ 화면에 쌓인 수정 이력에서 뽑아 쓴다. 1번이 선행되어야 재료가 생긴다.

**3. Anthropic API를 비교군에 추가**

`core/llm.py`의 `judge()`에 provider 분기를 추가한다(약 15줄). 월 1,500원 수준이며 판정이 10초 안쪽으로 끝난다. 로컬 4b가 천장인지 확인하는 용도.
